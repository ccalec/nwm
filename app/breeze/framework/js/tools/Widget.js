...
* @author <a href="http://">jianhui.fjh</a>
* @version 0.1
*/
...
define(function(require, exports, module) {
  ...
  _result.alert = function(text,type,timeout){
    ...
    htmlArr.push('<div class="widget-box zdy-alert">');
      htmlArr.push('<div class="widget-header header-color-blue">');
        htmlArr.push('<h5>系统提示</h5>');
        htmlArr.push('<span class="widget-toolbar">');
          htmlArr.push('<a href="javascript:void(0);" onclick="$.unblockUI();">');
            htmlArr.push('<i class="icon-remove white"></i>');
          htmlArr.push('</a>');
        htmlArr.push('</span>');
      htmlArr.push('</div>');
  ...
  _result.prompt = function(dom,title,succCallback,errCallback){
    ...
    htmlArr.push('<div class="widget-box zdy-prompt pull-left" id="promptBox">');
      htmlArr.push('<div class="widget-header header-color-blue">');
        htmlArr.push('<h5>'+title+'</h5>');
        htmlArr.push('<span class="widget-toolbar">');
          htmlArr.push('<a href="javascript:void(0);" class="J_propCancel">');
            htmlArr.push('<i class="icon-remove white"></i>');
          htmlArr.push('</a>');
        htmlArr.push('</span>');
      htmlArr.push('</div>');
        ...
        htmlArr.push('<div class="widget-main no-padding">');
          ...
          if(succCallback || errCallback){
            htmlArr.push('<div class="form-actions tac" style="margin:0;">');
              ...
              if(errCallback){
              htmlArr.push('<button type="button" class="btn btn-small btn-default J_propCancel">');
    ...
    newdom.find('#promptCon').append(dom);
    $('<div id="rq" style="width:1px; height:1px; overflow:hidden;"></div>').appendTo($('body'));
    $('#rq').html(newdom[0].outerHTML);
    var width = $('#rq>div').outerWidth();
    var height = $('#rq>div').outerHeight();
    $('#rq').remove();
    newdom.find('#propSubmit').on('click',function(){
      succCallback && succCallback();
    });
    newdom.find('.J_propCancel').on('click',function(){
