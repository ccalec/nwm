...
define(function(require, exports, module) {
  ...
  var _api = _result.f = {};
     ...
     */
     ...
     _api.createFormListByObjDesc = function(objdesc,dom,formData,callback,type){
     	var newDesc = {
			...
			for(var field in objdesc){
				if(field=='id') continue;
				newDesc[field] = objdesc[field];
			}
     	var islist = true;
     	var _objdesc = {
     		formList:{
    ...
    function beginCreateFormOrList(islist,objdesc,dom,formData,callback,type){
        //开始解析objdesc
        var formStr = "<div class='form-wrap ";//luo修改：这里取消了表单的提交按钮
        if(islist){
        	formStr += "islist ";
        }else{
        	formStr += "pull-left ";
		...
		function parserObjDesc($obj,$objName){
			for(var prop in $obj){
				...
				if(!$objName){
					...
					formStr+="<div class='control-group c_"+fType+" f_"+propForName+"'>";//每个表单的结构开始 luo修改：加上表单的类型
					if (fTitle!=""){//luo修改：如果没标题不要显示：
				...
				//==== 表示解析子循环title =============
					...
					//定义节点class
					var fName = "data." + propForName;
					var __fClass = fName.replace(/_dot_/gi,".").replace(/(\[.*?\])|(data)/g,"").replace(/\./g,"_").toLowerCase();//luo修改：对象间用_
					if(islist && !fIsList){
				...
				//==== 表示解析子循环内容 ====================
					...
					var __fClass = fName.replace(/_dot_/gi,".").replace(/(\[.*?\])|(data)/g,"").replace(/\./g,"_").toLowerCase().replace("formlist_","");//luo修改：对象间用_
					if(islist && !fIsList){
		...
		function parserField(fTitle,fType,fValueRange,fDesc,fWidth,fName,fValue,fIsList){
			...
			//定义节点class
			var fClass = fName.replace(/_dot_/gi,".").replace(/(\[.*?\])|(data)/g,"").replace(/\./g,"_").toLowerCase();//luo修改：对象间用_

			//判断是否有值，布尔状态
			formStr+="<!--$var valueStatus = true;-->";
			formStr+="<!--$try{ if(!"+fValue+" && "+fValue+" != 0){ valueStatus = false;}} catch(e){ valueStatus = false; };-->";
			...
			switch(fType){
				...
				//hidden隐藏的表单类型
				case "Hidden":
				    formStr+= "<input id='"+fName+"' class='inp_text "+fClass+"_inp' type='hidden' name='"+fName+"' ";		//2013-03-20 jianhui.fjh修改
				    formStr+="<!--$if(valueStatus){-->";
					formStr+="value='${lang:formatQuota("+fValue+")}' ";
					formStr+="<!--$}-->";
					formStr+="/>";
				...
				case "DatePicker":
					...
					if(type!=2){
						formStr+=" onClick='WdatePicker({el:\""+fName+"\"})'";
					}
					formStr+=">";
					...
					formStr+="class='date-picker' />";
					formStr+="<span class='add-on'><i class='icon-calendar'></i></span></div>";
					break;
				...
				case "TimePicker":
					...
					if(type!=2){
						formStr+=" onClick='WdatePicker({el:\""+fName+"\", dateFmt:\"HH:mm:ss\"})'";
					}
					formStr+=">";
					formStr+="<input id='"+fName+"' type='text' name='"+fName+"' ";
					formStr+="<!--$if(valueStatus){-->";
					...
					formStr+="class='date-picker' />";
					formStr+="<span class='add-on'><i class='icon-time'></i></span></div>";
					break;
				...
				case "DateTimePicker":
					...
					if(type!=2){
						formStr+=" onClick='WdatePicker({el:\""+fName+"\", dateFmt:\"yyyy-MM-dd HH:mm:ss\"})'";
					}
					formStr+=">";
					formStr+="<input id='"+fName+"' type='text' name='"+fName+"' ";
					formStr+="<!--$if(valueStatus){-->";
				...
				case "Radio":
					for (var i=0; i<fValueRange.length; i++){
						for(var emun in fValueRange[i]){
							formStr+="<label><input type='radio' class='J_RadioInput' name='"+fName+"' value='"+fValueRange[i][emun]+"' ";
							formStr+="<!--$if(valueStatus){-->";
				...
				case "List":
					...
					//递归解析 列表title名称;
					//jianhui.fjh 判断是否是字符串类型

					if($.isArray(fValueRange)){  //如果是list列表型
						formStr+="<thead>";
						formStr+="<tr><th style='width:60px;' class='center th_checkbox'>选择</th>";
						parserObjDesc(fValueRange[0],"$listTit");
						//存在列表，且是默认数据列表type存在，或等于0的情况，才有操作列
						if(islist && !type){
							formStr+="<th class='th_do' style='width:120px;'>操作</th>";
						}
						formStr+="</tr></thead>";
					}
					formStr+="<tbody>";
					...
					if(islist && type==2){
						//用于关联列表的时候，用单选框
						formStr+="<tr><td style='width:60px;' class='center td_checkbox'><label><input name='rowRadio' type='radio'/><span class='lbl'></span></label></td>";
					}else{
						//非列表或默认列表的时候，用复选框
						formStr+="<tr><td style='width:60px;' class='center td_checkbox'><label><input type='checkbox'/><span class='lbl'></span></label></td>";
					}
					if(islist && !type){
						//除关联列表视图，添加操作按钮列区域
						var btnHtml = "";
				     	btnHtml+="<td class='td_do'><div class='hidden-phone visible-desktop btn-group actionBtnForList'></div></td>";
				     }
					//递归解析
					if($.isArray(fValueRange)){  //如果是list列表型
					...
					//输出空白表单 用于增加一行操作
					formStr+="<tr class='list-tr-hidden' style='display:none;'><td style='width:60px;' class='center td_checkbox'><label><input type='checkbox'/><span class='lbl'></span></label></td>";
					if($.isArray(fValueRange)){  //如果是list列表型
						for(var prop in fValueRange[0]){
							if(fValueRange[0][prop].type=='ReadOnly'){
								fValueRange[0][prop].type = 'Text';
							}
						}
					...
					formStr += "<div style='width:220px;' class='ploptions hidden-phone visible-desktop btn-group pull-left'>";
					if(type!==2){
						//增加按钮
				...
				case "Object":
					...
					//增加编辑按钮
					formStr+="<a style='margin-top:5px;' alt='"+fTitle+"' class='btn btn-mini btn-info edit J_ObjectEdit' href='javascript:void(0);'><i class='icon-edit bigger-120'> 点击编辑</i></a>";
					break;
				...
				case "Langs":
					...
					//增加编辑按钮
					formStr+="<a style='margin:5px 0 0 10px;' alt='"+fTitle+"' class='btn btn-mini btn-info edit J_ObjectEdit' href='javascript:void(0);'><i class='icon-edit bigger-120'> 点击编辑</i></a>";
					break;
		...
		function formAction(){
			...
			function bindOuterLink(_dom){
				_dom.delegate('.outerLinkEdit','click',function(){
					//获取参数一：fieldName
			...
			//绑定选中第一个radio
			dom.find('.c_Radio').each(function(){
			...
			//======增加一行操作=======
			dom.delegate(".btn-add-con","click",function(){
			...
			if(!islist || (islist && type==1)){
				//======删除操作=======
				dom.delegate(".btn-del","click",function(){
			...
			//======全选操作=======
			dom.delegate(".btn-sel-all","click",function(){
			...
			//======反选操作=======
			dom.delegate(".btn-sel-oppo","click",function(){
				$(this).parent().prev().find(">tbody>tr").not(".list-tr-hidden").each(function(){
					var cbox = $(this).find(">td:eq(0) input[type='checkbox']");
					cbox.click();
				});
	      ...
	      NW.saveFiles(files, function(fileurl){
			...
			dom.find('.J_ObjectEdit').on('click',function(){
				...
				setTimeout(function(){
					//数据节点
					var domJosn = $(self).prev();

					//获得对话框标题：
					var titDialog = $(self).attr("alt");
					...
					fWid = Math.max(800, fWid);
					fHei = Math.max(100, fHei);
			...
			// 给生成的表单赋予getDate函数
			dom[0].getData=function(){//luo修改：不要把获取数据方法，绑定到表单提交上
				...
				function checkValueFun(checkObj,checkDesc){
					for(var name in checkDesc){
						...
						for (var i=0;i<descObj.valueRange.length;i++){
							...
							//2013-11-23luo修改，这里要重新使用会原来的功能，当然function是一个字符串，同样json也是一个字符串
							...
							if(checkOne.checkers ){
								if (/function/i.test(typeof(checkOne.checkers)) || /^function/i.test(checkOne.checkers)){
									...
									if (!funCheck(__data,checkValue)){
										//没有通过
										API.widgetAPI.alert(checkOne.failTips,'warning');
										return null;
									}
								}else{
	...
	_api.createJsonByForm = function(dom,objdesc){
		...
		$.each(arr, function() {
			...
			if(/checkbox/i.test(this.type)){   //新增类型的处理 Alec 20130630 checkbox
				eval("if(!"+name+"){ "+name+"=[];}");
