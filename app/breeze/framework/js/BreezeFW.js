	...
	function _createPost(){
		...
		return {
			...
			doPost:function(__finishedCallback,__this){
				...
				//EDIT:  改造是多个还是单个
				var ajaxdata = _postData.length==1?_postData[0]:_postData;
				//2013-09-14修改
				var NW = process.mainModule.exports;
				NW.doService(ajaxdata, function(__returndata){
					//EDIT:  改造是多个还是单个
	...
	function _createFWTools(__app){
		...
		*/
		return {
			...
			*/
			show:function(__viewId,__data,__target,__transition){
				//先将数据记录到本地存储中
				if (__data){
				...
				if (src == null){
					if(__transition){
						target.hide().html(__viewId).fadeIn(300);
					}else{
				...
				if(__transition){
					target.hide().html(htmlStr).fadeIn(100);
					return;
				}
				target.html(htmlStr).fadeIn(150);
			},
			/**
			*@function
			*@name showBack
