	...
	FW.register(
		{
			...
			param:{
				...
				*@name id
				*@memberOf cmsMgrGadget
				*@description 默认的id,通常和action:conEdit搭配出现
				*/
				id:"",
				/**
				*@name nodeid
			...
			*/
			...
			private:{
				...
				*/
				privateSetGlobalVar: function(){
					...
					_this.MY.serverName = {
						...
						dCon: "delData",     		//内容表:删
						mCon: "modData",			//内容表:改
						qCon: "queryData",   	    //内容表:查
						aModel: "addModel",			//cms模型：增
						dModel: "delModel",		//cms模型：删
						mModel: "modModel",		//cms模型：改
						qModel: "queryModel"      //cms模型：查
					}
				},
				/**
				*@function
					...
					_this.API.doServer(_this.MY.serverName.qModel, _this.MY.package, param,function(code,data){
						...
						if(code>0 && data){
							...
							for (var i=0; i<data.length; i++) {
								tabHtml += "<li>";
								tabHtml += "<a href='javascript:void(0)' sonalias='"+data[i].alias+"' data-toggle='tab'>"+data[i].model_name+"</a>";
								tabHtml += "</li>";
							}
							//并插入到对应节点中
				...
				*@ 2、给数据描述增加id的描述 this.MY.contentDesc[curAlias].id 赋值
				*@ 3、执行回调函数
				*@example
				*/
				...
				*/
				privateShowDefaultView: function(){
					...
					//内容编辑
					else if(this.MY.action == this.MY.act.conEdit){
						FW.trigerEvent("trigerReShowNodeTree",this.MY.alias);
				...
				*/
				privateOutLinkCB: function(_modelDesc, _formDom, _fieldName, _fieldValue, _callback){
					...
					//获得外联表的alias及主键字段名
					var l_outerLink = _modelDesc[_fieldName].outerLink;
					if(l_outerLink.split(".").length !== 2) return;
					var l_alias = l_outerLink.split(".")[0];
					var l_keyField = l_outerLink.split(".")[1];
					//设置弹出层默认标题
					var dialogTitle = "";
					//参数：param
					var param = {
					...
					_this.API.private("privateSetOutlinkView", newViewDom, param, sn, function(wrapConListDom, newParam){
						...
						reShowListForOutlink(wrapConListDom,newParam,function(){
							...
							_this.API.private("privateMaskLayer",newViewDom,dialogTitle,1000,function(){
								...
								if(!newData) return;
								//将data数据内容一一赋值进入外联字段
								for(var l_prop in _modelDesc){
									if (!_modelDesc[l_prop].outerLink){
										continue;
									}
									var arr_outerLink = _modelDesc[l_prop].outerLink.split(".");
									if(arr_outerLink.length !== 2) return;
									var ll_alias = arr_outerLink[0];
									var ll_keyField = arr_outerLink[1];
				...
				*/
				privateMessOutLinkOk: function(_modelDesc, _formDom, _fieldName, _fieldValue){
				...
				*/
				privateMaskLayer: function(_dom,_dialogTitle,_width,_bindBtnEvent){
					...
					htmlstr += "<div class='modal-body'><form id='outerLinkEditForm' class='form-horizontal outlinkform_"+this.MY.alias+" clearfix' style='margin:0px;'></form></div>";
					if(_bindBtnEvent){
						...
						if(toString.apply(_bindBtnEvent) === '[object Array]'){
												...
												function reShowOutLink(_data){
													...
													_this.API.private("privateSetDescAndData", _param.alias, _data, function(){
														...
														$.each(newDesc, function(fieldName, fd){
															...
															// 单选类型
															if($.inArray(fd.type,['Select','Radio'])!=-1){
																fd.type='Text';
															...
															}else if($.inArray(fd.type,['CheckBox','MultSelect'])!=-1){ //多选类型
																fd.type='Text';
																valueRange = fd.valueRange[0];
																$.each(_data,function(i,itemData){
																	if(!itemData[fieldName]) return true;
																	$.each(valueRange,function(name,value){
																		var idx = $.inArray(value, itemData[fieldName]);
																		if(idx!=-1){
																			itemData[fieldName][idx] = name;
																		}
																	})
																	itemData[fieldName] = itemData[fieldName].join('、');
																})
														...
														//执行内容列表回调函数
														_callback && _callback(data.listData);
													})
												}
												reShowOutLink(data.listData);
											})
										}
									})
								}
				...
				* 2、根据是否存在sonAlias，判断当前获取id还是sonId
				* 3、显示提交、返回按钮
				* 4、定义当前视图指针
				* 5、替换操作标题
				* 6、显示子级sonAlias的Tab
						...
						(function(iii){
							_this.API.addPost(_this.MY.serverName.mCon,_this.MY.package,{
								...
								if(code>0){
									...
									FW.use('Widget').alert("Cid="+data[iii].id+":修改失败！","danger");
								}
							});
						})(i)
					}
				...
				privateSubmitConPLAdd: function(_callback){
					...
					_this.API.doServer(_this.MY.serverName.aCon, _this.MY.package, _param, function(code,data){
						if(code>0 && data){
							...
							FW.use('Widget').alert("数据添加失败！","danger");
						}
					});
				},
				/**
				...
				privateSubmitClassEdit: function(_callback){
					...
					_this.API.doServer(_this.MY.serverName.mCon, _this.MY.package, param, function(code,data){
						if(code>0){
							...
							FW.use('Widget').alert("修改栏目保存失败！","danger");
						}
					})
				},
				/**
				...
				privateSubmitClassAdd: function(_callback){
					...
					_this.API.doServer(_this.MY.serverName.aCon, _this.MY.package, param, function(code,data){
						if(code>0 && data){
							...
							FW.use('Widget').alert("添加栏目保存失败！","danger");
						}
					})
				},
				/**
				*@function
				*@name privatePushState
				*@memberOf cmsMgrGadget
				*@description cms试图切换时，修改history纪录和地址栏url
				*/
				privatePushState: function(paramObj){
					// var curl = location.href;
					// for(var param in paramObj){
					// 	var cval = FW.use().getParameter(param);
					// 	var cpstr = param+'='+cval;
					// 	var tpstr =  param+'='+paramObj[param];
					// 	if(!paramObj[param]){
					// 		curl = curl.replace('&'+cpstr,'').replace('?'+cpstr,'');
					// 		continue;
					// 	};
					// 	if(~curl.indexOf(cpstr)){
					// 		curl = curl.replace(cpstr, tpstr);
					// 	}else{
					// 		var sn = curl.indexOf('?')==-1?'?':'&';
					// 		curl += (sn+tpstr);
					// 	}
					// }
					// curl = curl.replace('?','&').replace('/manager/','/manager?path=');
			...
			TrigerEvent:{
				...
				*/
				trigerClassDel: function(){
					...
					if(confirm("确定要删除该栏目吗？")){
						this.API.doServer(this.MY.serverName.dCon, this.MY.package, param, function(code,data){
							if(code>0 && data){
								FW.use('Widget').alert("栏目删除成功！","success");
								//删除sonAlias
								this.MY.nodeid = '';
								//修改history和地址栏URL
								this.API.private("privatePushState",{
									nodeid: ''
								});
								setTimeout(function(){
									window.location.reload();
								...
								FW.use('Widget').alert("栏目删除失败！","success");
							}
						})
					}
				},
				...
				*/
				trigerCheckBoxClick:function(rowData){
					//FW.use('Widget').alert(FW.use().toJSONString(rowData),"warning");
				}
