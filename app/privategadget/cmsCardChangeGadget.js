  ...
  FW.register(
    {
      ...
      FireEvent:{
        ...
        fireSubmit: function(){
          ...
          if(!data) return;
          var param = {
            no: self.MY.cardInfo.no,
            new_no: data.new_no,
            comment: data.comment
          }
          self.API.doServer('cardChange', 'cms', param, function(code,data){
            if(code>0){
              FW.use('Widget').alert("换卡成功", "success", 3000);
              self.API.private('updateUserInfo');
              self.API.private('privateShowDefaultView',{curstatus: param.status});
            }else if(code==-1){
              FW.use('Widget').alert("换卡失败！此卡已存在，且是已未激活状态","danger", 2000);
            }else{
              FW.use('Widget').alert("换卡失败！","danger", 2000);
            }
          })
        },
        fireReset: function(){
      ...
      private:{
        privateShowDefaultView: function(data){
          ...
          FW.use().createForm(this.MY.desc, this.MY.formDom);
        },
